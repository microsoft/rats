...
class TestPipelineSettingsClient:
    ...
    def test_basics(self) -> None:
        self._client.set(SettingName("foo1"), "bar")
        assert self._client.get(SettingName("foo1")) == "bar"

        with pytest.raises(DuplicatePipelineSettingError):
            self._client.set(SettingName("foo1"), "baz")

        with pytest.raises(PipelineSettingNotFoundError):
            self._client.get(SettingName("foo2"))
